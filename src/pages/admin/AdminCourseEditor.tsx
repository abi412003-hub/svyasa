...
interface CourseForm {
  slug: string; title: string; short_title: string; degree: string; duration: string;
  campus: string; campus_type: string; category: string; banner_image: string;
  hook_line: string; overview: string[]; apply_link: string; brochure_link: string;
  domain_theme: string; is_published: boolean;
  ...
  eligibility: { primary: string; minMarks: string; extras: string[]; quizQuestions: QuizQuestion[]; };
  careers: CareerPath[];
  testimonials: Testimonial[];
  fee: { registration: string; yearlyFees: YearlyFee[]; };
}
...
const EMPTY: CourseForm = {
  ...
  stat_callout: null, highlights: [], quote: null, learning_journey: [],
...
const AdminCourseEditor = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setForm({
        ...
        degree: data.degree ?? "", duration: data.duration ?? "",
        campus: data.campus ?? "", campus_type: data.campus_type ?? "gcc",
        category: data.category ?? "", banner_image: data.banner_image ?? "",
        hook_line: data.hook_line ?? "",
        overview: Array.isArray(data.overview) && data.overview.length > 0 ? data.overview as string[] : [""],
        apply_link: data.apply_link ?? "", brochure_link: data.brochure_link ?? "",
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      eligibility: { ...form.eligibility, extras: form.eligibility.extras.filter(e => e.trim()) },
      fee: { registration: form.fee.registration, yearlyFees: form.fee.yearlyFees.filter(f => f.amount.trim()) },
    };
    let error;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8 sticky top-0 z-10 bg-gray-50/90 backdrop-blur-sm py-4 -mx-8 px-8">
        <div className="flex items-center gap-3">
          <Link to="/admin/courses" className="text-gray-400 hover:text-gray-700 transition-colors">
            <ArrowLeft size={18} />
          </Link>
          <div>
            <h1 className="text-xl font-bold text-gray-900">{isNew ? "Add Course" : "Edit Course"}</h1>
      ...
      <div className="space-y-6">
        ...
        <Section title="Classification">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label className="text-gray-600 text-sm mb-1.5 block">Category Slug</Label>
              <Input value={form.category} onChange={e => set("category", e.target.value)} placeholder="bca" className="bg-gray-50 border-gray-300 text-gray-900 placeholder:text-gray-400" />
            </div>
            <div>
              <Label className="text-gray-600 text-sm mb-1.5 block">Campus</Label>
              <Input value={form.campus} onChange={e => set("campus", e.target.value)} placeholder="Global City Campus" className="bg-gray-50 border-gray-300 text-gray-900 placeholder:text-gray-400" />
            </div>
          </div>
            ...
            {form.overview.map((para, i) => (
              <div key={i} className="flex gap-2">
                <Textarea value={para} onChange={e => { const u = [...form.overview]; u[i] = e.target.value; set("overview", u); }}
                  ...
                  className="flex-1 bg-gray-50 border-gray-300 text-gray-900 placeholder:text-gray-400 resize-none text-sm" />
        ...
        <Section title="Quick Facts / Highlights" action={
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <p className="text-xs text-gray-500 uppercase tracking-wider">Stat Callout (optional)</p>
            ...
            {form.highlights.map((h, i) => (
              <div key={i} className="bg-gray-50 rounded-lg p-4 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-500">Highlight {i + 1}</span>
        ...
        <Section title="Quote">
          <div className="space-y-3">
            ...
            {form.quote && (
              <>
                <div>
                  <Label className="text-gray-600 text-sm mb-1.5 block">Quote Text</Label>
        ...
        <Section title="Your Learning Journey" action={
          ...
          <div className="space-y-3">
            {form.learning_journey.length === 0 && <p className="text-gray-400 text-sm">No learning journey steps added yet.</p>}
            {form.learning_journey.map((step, i) => (
              <div key={i} className="bg-gray-50 rounded-lg p-4 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-500 flex items-center gap-1"><GripVertical size={12} /> Step {i + 1}</span>
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <Label className="text-gray-600 text-sm">Additional Requirements</Label>
              ...
              {form.eligibility.extras.map((ex, i) => (
                <div key={i} className="flex gap-2 mb-2">
                  <Input value={ex} onChange={e => { const u = [...form.eligibility.extras]; u[i] = e.target.value; set("eligibility", { ...form.eligibility, extras: u }); }}
                    ...
                    className="bg-gray-50 border-gray-300 text-gray-900 placeholder:text-gray-400 text-sm" />
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <Label className="text-gray-600 text-sm">Quiz Questions (Am I Eligible?)</Label>
              ...
              {form.eligibility.quizQuestions.map((q, i) => (
                <div key={i} className="bg-gray-50 rounded-lg p-3 mb-2 flex gap-3 items-start">
                  <div className="flex-1">
                    <Input value={q.question} onChange={e => { const u = [...form.eligibility.quizQuestions]; u[i] = { ...u[i], question: e.target.value }; set("eligibility", { ...form.eligibility, quizQuestions: u }); }}
                      ...
                      className="bg-white border-gray-300 text-gray-900 placeholder:text-gray-400 text-sm mb-1" />
        ...
        <Section title="Where This Degree Takes You" action={
          ...
          <div className="space-y-3">
            {form.careers.length === 0 && <p className="text-gray-400 text-sm">No career paths added yet.</p>}
            {form.careers.map((c, i) => (
              <div key={i} className="bg-gray-50 rounded-lg p-4 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-500">Career {i + 1}</span>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    <Label className="text-gray-600 text-sm mb-1.5 block">Demand</Label>
        ...
        <Section title="Student Voices" action={
          ...
          <div className="space-y-3">
            {form.testimonials.length === 0 && <p className="text-gray-400 text-sm">No testimonials added yet.</p>}
            {form.testimonials.map((t, i) => (
              <div key={i} className="bg-gray-50 rounded-lg p-4 space-y-3">
                ...
                <div>
                  <Label className="text-gray-600 text-sm mb-1.5 block">Quote</Label>
      ...
      <div className="mt-8 flex justify-end">
        <Button onClick={handleSave} disabled={saving} className="gap-2 bg-amber-500 hover:bg-amber-600 text-white font-semibold px-8">
...
const AddBtn = ({ onClick, label }: { onClick: () => void; label: string }) => (
  <Button variant="ghost" size="sm" onClick={onClick} className="gap-1.5 text-gray-500 hover:text-gray-900 text-xs">
    <Plus size={12} /> {label}
  </Button>
);

const RemoveBtn = ({ onClick }: { onClick: () => void }) => (
  <button onClick={onClick} className="text-gray-400 hover:text-red-500 transition-colors self-start mt-2">
    <X size={14} />
  </button>
);
