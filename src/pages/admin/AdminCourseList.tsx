...
const AdminCourseList = () => {
  ...
  const fetchCourses = async () => {
    ...
    const { data, error } = await (supabase.from("courses") as any)
      .select("id,slug,title,short_title,category,campus_type,domain_theme,is_published,updated_at")
      .order("category")
      .order("title");
  ...
  const togglePublished = async (id: string, current: boolean) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase.from("courses") as any).update({ is_published: !current }).eq("id", id);
    if (error) toast({ title: "Error", description: error.message, variant: "destructive" });
    else setCourses(prev => prev.map(c => c.id === id ? { ...c, is_published: !current } : c));
  };
