...
export default function NewsForm() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-[#1e293b]">{isEdit ? "Edit News" : "Add News Article"}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="space-y-2">
            <Label>Title *</Label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} onBlur={() => !slug && setSlug(generateSlug(title))} placeholder="Enter news title" />
          </div>
          <div className="space-y-2">
            <Label>Slug</Label>
          ...
          <div className="space-y-2">
            <Label>Body</Label>
            <Textarea rows={6} value={body} onChange={(e) => setBody(e.target.value)} placeholder="Write the news article content..." />
          </div>
          <div className="space-y-2">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2"><Switch checked={isFeatured} onCheckedChange={setIsFeatured} /><Label>Feature on homepage</Label></div>
        ...
        <div className="lg:col-span-2">
          <Card className="shadow-sm sticky top-6">
            ...
            <CardContent className="space-y-3">
              ...
              {isFeatured && <Badge className="bg-[#e8731a] text-white">Featured</Badge>}
              <h3 className="font-bold text-lg text-[#1e293b] line-clamp-2">{title || "Untitled"}</h3>
              <p className="text-sm text-[#64748b]">{date ? fmtPreviewDate(date) : "No date"}</p>
      ...
      <div className="sticky bottom-0 bg-[#f8fafc] border-t py-3 flex gap-3 justify-end">
        <Button variant="ghost" onClick={() => navigate("/news-admin/news")} disabled={loading}>Cancel</Button>
